<div class="p-4 sm:p-6 space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
            <h1 class="text-3xl font-black text-espresso dark:text-cream uppercase tracking-tight">🎁 Katalog Reward
            </h1>
            <p class="text-coffee-600 dark:text-coffee-300 font-semibold mt-1">Tukarkan poin loyalty Anda dengan hadiah
                eksklusif!</p>
        </div>
    ...
    @if($successMessage)
        <div class="nb-card bg-matcha/20 p-4 flex items-start gap-3">
            ...
            <button wire:click="$set('successMessage', '')"
                class="text-espresso hover:text-berry font-black text-xl">×</button>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        @forelse($rewards as $reward)
            @php $canAfford = $user->total_poin >= $reward->poin_cost;
            $inStock = $reward->stok === null || $reward->stok > 0; @endphp
            <div
                class="nb-card bg-cream dark:bg-coffee-900 p-5 flex flex-col justify-between space-y-3 {{ !$inStock ? 'opacity-60' : '' }}">
                <div>
                    ...
                    <p class="text-xs text-coffee-600 dark:text-coffee-300 mt-1 font-semibold line-clamp-2">
                        {{ $reward->deskripsi }}</p>
                </div>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center text-xs font-bold">
                        <span
                            class="nb-badge bg-caramel text-espresso text-sm">{{ number_format($reward->poin_cost, 0, ',', '.') }}
                            ⭐</span>
                        <span class="text-coffee-400 text-[10px] font-black">Stok:
                            {{ $reward->stok === null ? '∞' : $reward->stok }}</span>
                    </div>

                    @if(!$inStock)
                        <button disabled class="nb-btn w-full bg-zinc-300 text-coffee-500 cursor-not-allowed text-xs">Stok
                            Habis</button>
                    @elseif(!$canAfford)
                        <button disabled class="nb-btn w-full bg-zinc-300 text-coffee-500 cursor-not-allowed text-xs">Poin Tidak
                            Cukup</button>
                    @else
                        <button wire:click="redeem({{ $reward->id }})"
                            wire:confirm="Tukar {{ number_format($reward->poin_cost, 0, ',', '.') }} poin untuk '{{ $reward->nama }}'?"
                            class="nb-btn w-full bg-berry text-white text-xs">🎁 Tukar Sekarang</button>
                    @endif
                </div>
            </div>
        @empty
                    ...
                    @forelse($myRedemptions as $r)
                        <tr class="hover:bg-caramel/10 border-b border-black/10">
                            ...
                            <td class="px-4 py-3 text-right"><span
                                    class="nb-badge bg-matcha text-espresso">{{ $r->status }}</span></td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-coffee-400 text-sm font-bold">Anda belum
                                pernah menukar reward.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
